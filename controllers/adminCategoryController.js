...
exports.create = async (req, res) => {
  try {
    ...
    const { name, emoji, color, description, sort_order, type } = req.body;
    ...
    // Validate type
    const validTypes = ['cut', 'raw', 'none'];
    const categoryType = type && validTypes.includes(type) ? type : 'none';
...
exports.update = async (req, res) => {
  try {
      ...
      if (category.image_url) {
        ...
        if (fs.existsSync(oldPath)) {
          try { 
            ...
            console.log('⚠️ Error deleting old image:', e.message);
          }
        }
      }
    ...
    if (type !== undefined) {
      const validTypes = ['cut', 'raw', 'none'];
      updateData.type = validTypes.includes(type) ? type : 'none';
    }
    ...
    console.log('✅ Category updated:', category.id, 'Type:', updateData.type || category.type);
    res.json({ success: true, data: updatedCategory });
  } catch (err) {
    console.error('❌ Error updating category:', err);
    if (req.file && fs.existsSync(req.file.path)) {
    ...
    if (category.image_url) {
      ...
      if (fs.existsSync(imagePath)) {
        try { 
          ...
          console.log('⚠️ Error deleting image:', e.message);
        }
      }
    }
