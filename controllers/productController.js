...
exports.getAll = async (req, res, next) => {
  try {
    ...
    // ── Category filter ──
    // Only apply category filter if both category and category_id are provided
    // Otherwise, return all products with their categories
    const categoryWhere = {};
    if (category) {
      categoryWhere.name = { [Op.like]: `%${category}%` };
    }
    ...
    // Build include with or without category filter
    const includeConfig = [
      {
        model: Category,
        as: 'category',
    ...
    // Only add category where clause if filters are provided
    if (category || category_id) {
      includeConfig[0].where = categoryWhere;
    }
...
/**
 * GET /api/products/featured
 * Returns top-rated / best-seller products
 */
exports.getFeatured = async (req, res, next) => {
  try {
    const products = await Product.findAll({
